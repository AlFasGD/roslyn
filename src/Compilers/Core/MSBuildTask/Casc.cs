...
{
    /// <summary>
    /// This class defines the "Casc" XMake task, which enables building assemblies from CA#
    /// source files by invoking the CA# compiler. This is the new Roslyn XMake task,
    /// meaning that the code is compiled by using the Roslyn compiler server, rather
    /// than casc.exe. The two should be functionally identical, but the compiler server
    /// should be significantly faster with larger projects and have a smaller memory
    /// footprint.
    /// </summary>
    public class Casc : ManagedCompiler
    {
        #region Properties

        // Please keep these alphabetized. These are the parameters specific to Casc.
        ...
        /// </summary>
        /// <remarks>
        /// If set to null, "/preferreduilang" option is omitted, and casc.exe uses its default setting.
        ...
        protected internal override void AddResponseFileCommands(CommandLineBuilderExtension commandLine)
        {
            ...
            {
                throw new InvalidOperationException(string.Format(ErrorString.General_IncorrectHostObject, "Casc", "ICscHostObject"));
            }
            if (!designTime)
            {
        ...
        internal override RequestLanguage Language => RequestLanguage.CASharpCompile;

        /// <summary>
        /// The CA# compiler (starting with Whidbey) supports assembly aliasing for references.
        ...
        /// command-line on casc.exe.  The syntax for aliasing a reference is:
        ///     casc.exe /reference:Goo=System.Xml.dll
        ///
            ...
            bool isInteractive = false)
        {
            ...
            foreach (ITaskItem reference in references)
            {
                ...
                else
                {
                    ...
                    // we're going to add a separate /reference: switch to the casc.exe
                    // command-line
                    ...
                    foreach (string alias in aliases)
                    {
                        ...
                        // The alias should be a valid CA# identifier.  Therefore it cannot
                        ...
                        // that are not allowed in a CA# identifier, but we'll just let casc.exe
        ...
        /// 
        /// Note that CA# doesn't support assigning a value to the constants ... in
        ...
        internal static string? GetDefineConstantsSwitch(string? originalDefineConstants, TaskLoggingHelper log)
        {
            ...
            // Loop through all the parts, and for the ones that are legal CA# identifiers,
            ...
            {
                // We wouldn't want to pass in an empty /define: switch on the casc.exe command-line.
                return null;
            }
        }
        ...
        ///     thing for casc.exe, the IDE compiler cannot support it.  In this situation
        ...
        private bool InitializeHostCompiler(ICscHostObject cscHostObject)
        {
            ...
            try
            {
                // Need to set these separately, because they don't require a CommitChanges to the CA# compiler in the IDE.
        ...
        /// <summary>
        /// This method will get called during Execute() if a host object has been passed into the Casc
        ...
        protected override HostObjectInitializationStatus InitializeHostObject()
        {
            if (HostObject != null)
            {
                ...
                // "Object" (because ITask interface obviously can't have any Casc-specific stuff
                // in it, and each task is going to want to communicate with its host in a unique
                // way).  Now we cast it to the specific type that the Casc task expects.  If the
                // host object does not match this type, the host passed in an invalid host object
                // to Casc, and we error out.
                ...
                if (HostObject is ICscHostObject hostObjectCOM)
                {
                    using (RCWForCurrentContext<ICscHostObject> hostObject = new RCWForCurrentContext<ICscHostObject>(hostObjectCOM))
                    {
                        ...
                        if (!this.HostCompilerSupportsAllParameters)
                        {
                            ...
                            // Vbc / Casc / etc.  Actually, the real reason is bug 275726 (ddsuites\src\vs\env\vsproject\refs\ptp3).
                ...
                {
                    Log.LogErrorWithCodeFromResources("General_IncorrectHostObject", "Casc", "ICscHostObject");
                }
            }
